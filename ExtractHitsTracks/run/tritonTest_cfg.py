...
models = {
    "GNN4TrackingProducer": ['GNN4Tracking'],
}
...
options = VarParsing()
options.register("maxEvents", 100, VarParsing.multiplicity.singleton, VarParsing.varType.int, "Number of events to process (-1 for all)")
...
options.register("address", "ailab01.fnal.gov", VarParsing.multiplicity.singleton, VarParsing.varType.string, "server address")
...
options.register("modules", "GNN4TrackingProducer", VarParsing.multiplicity.list, VarParsing.varType.string, "list of modules to run (choices: {})".format(', '.join(models)))
options.register("models","GNN4Tracking", VarParsing.multiplicity.list, VarParsing.varType.string, "list of models (same length as modules, or just 1 entry if all modules use same model)")
    ...
    setattr(process, module,
        Module(module,
            Client = cms.PSet(
                ...
                modelConfigPath = cms.FileInPath("test/ExtractHitsTracks/data/models/{}/config.pbtxt".format(model)),
